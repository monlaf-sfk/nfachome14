...
from src.users.models import User as UserModel # Import the ORM model for type hinting current_user
from . import schemas, service # service and models from this 'tasks' package
...
async def create_task_for_current_user(
    ...
    return await service.create_user_task(db=db, task=task, user_id=current_user.id)
...
async def update_single_task(
    ...
    if db_task_to_update is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Task not found")
    return await service.update_task(db=db, db_task=db_task_to_update, task_update=task_update)
...
async def delete_single_task(
    ...
    return None # Or Response(status_code=HTTP_204_NO_CONTENT)